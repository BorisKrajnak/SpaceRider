...
def run(screen):
    ...
    def get_grade_color(number):
        if number == 1: return (0, 255, 0)
        if number == 2: return (170, 255, 0)
        if number == 3: return (255, 165, 0)
        if number == 4: return (255, 85, 0)
        return (255, 0, 0)
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_ESCAPE: return GameState.MENU
                if event.key == pygame.K_m: toggle_mute()
            if event.type == pygame.MOUSEBUTTONDOWN:
                if music_button_rect.collidepoint(event.pos):
                    toggle_mute()
            if event.type == SPAWN_EVENT:
                number = random.randint(1, 5)
                rect = pygame.Rect(width + 50, random.randint(50, height - 50), 0, 0)  # len text
